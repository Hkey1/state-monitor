...
class Table extends Tabs {	
	...
	castChildItem(opts, name=undefined){
		...
		if(typeof(opts)==='object' && !Array.isArray(opts)){
			if(opts instanceof GroupBy){
				isGroupBy = true;
			} else if(opts.groupBy){
				isGroupBy = true;
			} else if(opts.filter && typeof(opts.filter)==='object' && !Array.isArray(opts.filter)){
				if(opts.filter instanceof GroupBy){
					isGroupBy = true;
				} else if(opts.filter.groupBy){
					isGroupBy = true;
				}
			}
		}		
